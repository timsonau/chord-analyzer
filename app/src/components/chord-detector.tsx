            ...
            <div key={match.name} className="space-y-1">
              ...
              <div className="text-xs text-muted-foreground flex flex-wrap gap-1 mt-1">
                {match.notes.map((note) => (
                  <span
                    key={note}
                    className={`${activeNotes.includes(note) ? "text-primary" : "text-muted-foreground"}`}
                  >
                    {note}
                  </span>
                ))}
  ...
  for (const root of possibleRoots) {
    for (const [chordType, chordInfo] of Object.entries(chordDatabase)) {
      ...
      chordNotes.forEach((note, index) => {
        ...
        const isThird =
          chordInfo.intervals[index] === 4 || chordInfo.intervals[index] === 3;
        const isSeventh =
          chordInfo.intervals[index] === 10 ||
          chordInfo.intervals[index] === 11;
